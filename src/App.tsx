...
function Create({ onDone, onClose }: { onDone: () => void; onClose: () => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await api.create({ title: form.get('title'), description: form.get('description'), image_key: upload.image_key, starting_price: Number(form.get('price')), auction_end_at: new Date(String(form.get('end'))).toISOString() })
      onDone()
